...
c.execute('''CREATE TABLE IF NOT EXISTS leaderboard(username TEXT, score INTEGER)''')
c.execute('''CREATE TABLE IF NOT EXISTS songs(listno TEXT, artist TEXT, song TEXT)''')
...
try: f = open('songlist.csv','r')
    ...
    try: urllib.request.urlretrieve('https://raw.githubusercontent.com/SaabirS/Music-Quiz/master/program.py','songlist.csv')
    except:                                 
        print("\nPlease allow connections to https://github.com/SaabirS/Music-Quiz to download files")
        raise SystemExit
...
f.close()
db.commit()
#Creating a function for the signin options
def signin_options():
    signin = ''
